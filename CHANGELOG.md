...
## v3.14.0 (2026-09-24) — healthlite 运行史审计（信息雷达驱动）

本版由「信息雷达」情报驱动：WorkBuddy 5.6.2 修复了定时任务「**没有运行记录却反复执行**」
（2026-09-24 简报），暴露出一类通用盲区——**调度类任务「执行了却没留痕 / 反复执行却无人察觉」**。
超脑自己的 L0 守护（sb_healthlite，跑在系统计划任务上）同属该类系统：v3.12.2 起
`health_history.json` 负责留痕（生产侧），但一直没有**消费侧核查**——本版补齐这缺失的另一半。

### 新增 — `audit_history()` + `--history` / `--days` CLI

- `sb_healthlite.py --history [--days N]`：对 `health_history.json` 做执行节奏审计（只读）。
- 三类异常检出：
  - **守护停滞**：最近一次运行距今 > 2 天
  - **断档**：相邻两次运行间隔 > 48h
  - **疑似重复执行**：相邻两次运行间隔 < 60min（对应「反复执行」盲区）
- 行为契约：只读、**不取锁**（不与守护实例互相干扰）、恒退出码 0；
  历史文件缺失/损坏/非数组/条目时间戳畸形一律容错（no_history 或跳过），绝不因审计而崩溃。
- 输出含窗口内运行次数、首末次时间、距今天数与逐项 warn 明细。

### 设计说明：为什么不新增第二份运行日志

`health_history.json`（90 条 cap，含 ts/status/n_issues/duration_sec）在字段层面已等价于
最初设想的 `run_log.jsonl`。再写一份并行日志是冗余数据源；盲区从来不在「没有记录」，
而在「没有检查记录的东西」——故本版只做消费侧。

### 真实数据首次冒烟（验收的一部分）

对本机数据首跑即命中真实异常：`health_history.json` 仅 2026-08-31 的 4 条记录，
此后 23 天无 healthlite 运行痕迹（守护停滞 warn；同日另有 2 处 <60min 间隔判为疑似重复，
系当日开发联调）。**这正是该功能要消灭的盲区类型**——此前无人察觉。

### 验证

- 新增 `test_healthlite_audit.py`（12 项：空/缺/损坏历史、正常节奏、停滞、断档、
  疑似重复、边界 61min、窗口截断、畸形条目容错、naive 时间戳兜底），全绿
- 回归：全量测试套件在中性路径副本上重跑，全绿
- 版本声明面五点一致（sb_core / SKILL.md frontmatter / SKILL.md 标题 / README / CHANGELOG）
- 纯标准库零依赖；**不动任何检索常量**（BM25_K1 / B / ENTITY_HIT_MULTIPLIER / CJK_UNIGRAM_WEIGHT）
...
## v3.12.2 (2026-08-31) — 工作台全线（写通道+看板+易用性改造）+ 阶段2 整合/时态 + 图谱消融

### 新增 — 本地工作台 易用性改造（产品视角，下午增量）
