...
修复会话生命周期 T2 协议的根本架构缺陷——触发不再依赖 Agent 记忆，改为强制规则 + 四类阶段转换信号检测。

- **强制规则 #6**：SKILL.md 新增第六条强制规则，要求 Agent 主动监测对话阶段转换信号
- **四类触发信号**：精力信号（疲劳/不想继续）、话题转向（执行→反思/闲聊）、里程碑达成（结论后无新推进）、自然断开（结束语+无新话题）
- **T2 协议三步**：selfcheck --fix + token-roi --quickline + 追加工作日志
- **关键原则**：触发责任在 Agent 不在用户；跑完 T2 后正常回复，技术输出只写日志不进聊天

### 修复 — Obsidian wikilink 构建增强

- `sb_obsidian.py` `_build_wikilinks()` 从单一图谱边源升级为五源优先级链接
- 新增实体交叉链接（同 entity 的记忆自动互链）
- 新增标签社区链接（50%+ 标签重叠的记忆自动互链）
- 新增 vault 笔记自动链接（entity 匹配已有 vault note 标题）
- 修复 v3.x+ ingest 孤立问题（无图谱边的记忆也能通过实体/标签获得连接）
